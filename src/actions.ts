...
export async function postCard(cards: any) {
  ...
  const response: Response = await fetch(
    "http://localhost:8081/api/create/speciality",
    {
      method: "POST",
...
export async function deleteCard(id: any) {
  console.log(id);
  const response: Response = await fetch(
    "http://localhost:8081/api/delete/speciality",
    {
      method: "DELETE",
      headers: {
...
export async function putCard(id: any, name: any, physician_in_charge: any) {
  console.log(id);
  const response: Response = await fetch(
    `http://localhost:8081/api/update/${id}?name=${name}&physician_in_charge=${physician_in_charge}`,
    {
      method: "PUT",
      headers: {
        "Content-Type": "application/json",
